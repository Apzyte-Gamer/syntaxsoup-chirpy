...
I copy the `id_rsa` and `known_hosts` to my `~/.ssh` folder.
...
Looking at the contents none of the files jump out at me, mostly just txt files of documentation and a very useless README. But, inside the `.git` folder are some pack files. Using [`packfile_reader`](https://github.com/robisonsantos/packfile_reader) we can extract the data to some text files.
